...
function collapse(el) {
  ...
  on(el, 'click', function (ref) {
    ...
    for (var cur = target.nextElementSibling; cur; cur = cur.nextElementSibling) {
      toggleClass(cur, 'collapse');
    }
  });
}
...
function getAndActive(router, el, isParent, autoTitle) {
  ...
  links.sort(function (a, b) { return b.href.length - a.href.length; }).forEach(function (a) {
    ...
    if (hash.indexOf(href) === 0 && !target) {
      ...
      for (var i = node; i !== el; i = i.parentNode) {
        if (i.classList) {
          toggleClass(i, 'remove', 'collapse');
        }
      }
    } else {
...
function fetchMixin(proto) {
  ...
  proto._loadSideAndNav = function (path, qs, loadSidebar, cb) {
    ...
    return function () {
      ...
      // Load sidebar
      get(
        this$1.router.getFile('/' + loadSidebar) + qs,
        false,
        this$1.config.requestHeaders
      ).then(fn);
    }
  };
