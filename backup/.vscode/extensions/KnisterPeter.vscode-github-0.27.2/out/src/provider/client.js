...
function createClient(git, tokens, uri, logger) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const allowUnsafeSSL = !!helper_1.getConfiguration('github', uri).allowUnsafeSSL;
        if (!tokenInfo) {
            throw new Error(`No token found for host ${hostname}`);
        }
        switch (tokenInfo.provider) {
            case 'github':
                return new client_1.GithubClient(protocol, hostname, tokens[hostname].token, logger, allowUnsafeSSL);
            case 'gitlab':
                return new client_2.GitLabClient(protocol, hostname, tokens[hostname].token, logger, allowUnsafeSSL);
            default:
