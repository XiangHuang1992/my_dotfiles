...
import { addListener, removeListener } from './global-state';
...
var TSDI = /** @class */ (function () {
    ...
    TSDI.prototype.destroyInstance = function (idx, metadata) {
        ...
        if (instance) {
            ...
            var destroy = Reflect.getMetadata('component:destroy', isFactoryMetadata(metadata) ? metadata.rtti : metadata.fn.prototype);
            if (destroy) {
    ...
    TSDI.prototype.registerComponent = function (componentMetadata) {
        ...
        if (this.components.indexOf(componentMetadata) === -1) {
            if (componentMetadata.options.name && findIndexOf(this.components, function (meta) { return meta.options.name === componentMetadata.options.name; }) > -1) {
        ...
        if (inject.options.lazy || inject.options.dynamic) {
            ...
            Object.defineProperty(instance, inject.property, {
                ...
                get: function () {
                    ...
                    var value = tsdi_1.getComponentDependency(inject);
    ...
    TSDI.prototype.injectAutoMock = function (instance, inject) {
        ...
        if (injectMetadata) {
            var constructor = isFactoryMetadata(injectMetadata) ? injectMetadata.rtti : injectMetadata.fn;
    ...
    TSDI.prototype.checkAndThrowDependencyError = function (inject) {
        if (inject.type && inject.options.name) {
            ...
            log('Known Components: %o', this.components.map(function (component) {
                return isFactoryMetadata(component) ? component.rtti.name : component.fn.name;
            }));
            throw e;
        }
        if (!inject.type || inject.options.name) {
    ...
    TSDI.prototype.getScope = function (name) {
        ...
        return {
            ...
            leave: function () {
                ...
                self.components
                    .filter(function (metadata) { return !isFactoryMetadata(metadata) && metadata.options.scope === name; })
                    .forEach(function (metadata) {
                    var idx = self.getComponentMetadataIndex(isFactoryMetadata(metadata) ? metadata.rtti : metadata.fn);
