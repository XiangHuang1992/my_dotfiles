...
class JavaProvider {
            ...
            for (const imp of raw.imports) {
                ...
                for (const line of lines) {
                    if (line === `import ${importDeclaration};`) {
                        position = new vscode.Range(new vscode.Position(lines.indexOf(line), 0), new vscode.Position(lines.indexOf(line), importDeclaration.length + 8));
                        break;
                    }
                }
                ...
                entityDefinition.methods = entity.bodyDeclarations
                    ...
                    const position = lines.map((line) => {
                        ...
                        if (line.trim().indexOf(expr) === 0) {
                            ...
                            return new vscode.Range(new vscode.Position(lines.indexOf(line), start), new vscode.Position(lines.indexOf(line), start + m.name.identifier.length));
                        }
                        return undefined;
                    ...
                    return {
                        arguments: m.parameters.map((p) => {
                            ...
                            const pos = new vscode.Range(new vscode.Position(position.start.line, s), new vscode.Position(position.start.line, s + p.name.identifier.length));
                ...
                entityDefinition.properties = entity.bodyDeclarations
                    ...
                    const position = lines.map((line) => {
                        ...
                        if (line.trim().indexOf(expr) === 0) {
                            ...
                            return new vscode.Range(new vscode.Position(lines.indexOf(line), start), new vscode.Position(lines.indexOf(line), start + name.length));
                        }
                        return undefined;
    ...
    generate(entityName, node, includeBody, options) {
        ...
        if (options.ns !== undefined) {
            edits.push(new vscode.TextEdit(new vscode.Range(new vscode.Position(edits.length, 0), new vscode.Position(edits.length, 1024)), `package ${options.ns};${os.EOL}${os.EOL};`));
        }
        if (includeBody && node.name.indexOf(":")) {
            // Attempt to import the interface
            edits.push(new vscode.TextEdit(new vscode.Range(new vscode.Position(edits.length, 0), new vscode.Position(edits.length, 1024)), `import ${node.name.split(":").reverse().join(".")};${os.EOL}${os.EOL};`));
        }
        ...
        edits.push(new vscode.TextEdit(new vscode.Range(new vscode.Position(edits.length, 0), new vscode.Position(edits.length, 1024)), `${definitionLine} {${os.EOL}`));
        ...
        if (node.methods !== undefined) {
            ...
            for (const method of methods) {
                ...
                edits.push(new vscode.TextEdit(new vscode.Range(new vscode.Position(edits.length + (includeBody ? 3 : 0), 0), new vscode.Position(edits.length + (includeBody ? 3 : 0), line.length)), line + os.EOL));
            }
        }
        edits.push(new vscode.TextEdit(new vscode.Range(new vscode.Position(edits.length + (includeBody ? 3 : 0), 0), new vscode.Position(edits.length + (includeBody ? 3 : 0), 1)), `}`));
        return edits;
    }
...
exports.JavaProvider = JavaProvider;
