...
function checkPythonExtensionInstalled() {
    ...
    if (semver.lt(pyExt.packageJSON.version, "2018.7.1")) {
        vscode.window.showErrorMessage(`Please update the Python extension to enable IntelliCode.\nIntelliCode requires version 2018.7.1 or newer.`);
        return false;
    }
    return true;
}
function checkPrereleaseNotInstalled() {
    let prereleaseExt = vscode.extensions.getExtension("ms-vsintellicode.vsintellicode");
    if (prereleaseExt === undefined) {
        return;
    }
    if (prereleaseExt.packageJSON.version === "1.0.0") {
        vscode.window.showWarningMessage('Please remove the prerelease version of IntelliCode (ms-vsintellicode.vsintellicode)');
    }
}
